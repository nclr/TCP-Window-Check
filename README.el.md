...
- **RTT (round-trip time)** — πόσο χρόνο χρειάζεται ένα πακέτο να φτάσει στον server και
  η επιβεβαίωση (ACK) να γυρίσει πίσω. Κάθε μηχανισμός παραθύρου TCP κλιμακώνεται με αυτό.
  *Παράδειγμα: Άμστερνταμ από Αθήνα ίσως ~45 ms· server στην ίδια πόλη ~2 ms.*
- **BDP (bandwidth-delay product, γινόμενο εύρους ζώνης–καθυστέρησης)** — `ρυθμός_γραμμής × RTT`:
  πόσα bytes πρέπει να είναι "στον αέρα" (σταλμένα αλλά μη επιβεβαιωμένα) κάθε στιγμή για να
  μένει η γραμμή γεμάτη.
  *Παράδειγμα: 1 Gbit/s × 45 ms = 125 MB/s × 0.045 s ≈ 5.6 MB. Κάτω από ~5.6 MB δεδομένων
  σε πτήση, αυτή η ζεύξη φυσικά δεν μπορεί να τρέξει στο 1 Gbit/s με μία ροή.*
- **Receive window (rwnd, παράθυρο λήψης)** — πόσα δεδομένα *εσείς* λέτε στον server ότι
  επιτρέπεται να στείλει πριν σταματήσει να περιμένει τα ACK σας. Διαφημίζεται σε κάθε
  πακέτο που στέλνετε πίσω· περιορίζεται από το `net.ipv4.tcp_rmem` του πυρήνα σας.
  *Παράδειγμα: με παράθυρο 2 MiB σε διαδρομή 45 ms, μια ροή κορυφώνεται στα
  2 MiB / 0.045 s ≈ 355 Mbit/s — όσο γρήγορη κι αν είναι η γραμμή.*
- **Send window / cwnd (παράθυρο συμφόρησης)** — το αντίστοιχο από την πλευρά του server:
  πόσα μη επιβεβαιωμένα δεδομένα είναι διατεθειμένος να κρατά σε πτήση ο *αποστολέας*,
  περιορισμένο από τον buffer αποστολής του (`tcp_wmem`) και τον αλγόριθμο ελέγχου συμφόρησης.
  *Παράδειγμα: server με όριο 0.6 MiB σε πτήση παραδίδει το πολύ 0.6 MiB / 0.045 s ≈
  107 Mbit/s σε εσάς, ακόμα κι αν το παράθυρό σας και η γραμμή σας είναι τεράστια.*
- **In-flight bytes (bytes σε πτήση)** — δεδομένα ήδη σταλμένα αλλά όχι ακόμα επιβεβαιωμένα,
  μετρημένα από την καταγραφή. Το peak δείχνει το μέγιστο που ρίσκαρε ποτέ ο server· το p95
  είναι μια σταθερότερη εκτίμηση.
- **Sustained in-flight** — `ταχύτητα × RTT`, το επίπεδο σε πτήση που *διατηρήθηκε* κατά
  μέσο όρο. Μεγάλο κενό μεταξύ peak και sustained σημαίνει εξάρσεις και υποχωρήσεις —
  σήμα απωλειών/ουρών.
- **Window scale (κλίμακα παραθύρου)** — το πεδίο παραθύρου του TCP είναι μόνο 16 bits
  (μέγιστο 64 KiB), οπότε οι δύο πλευρές συμφωνούν έναν πολλαπλασιαστή στο handshake.
  *Παράδειγμα: ακατέργαστη τιμή 49152 με κλίμακα 7 → 49152 × 2⁷ = 6 MiB.*
- **rcv_space** — η τρέχουσα εκτίμηση του πυρήνα για τον buffer λήψης που χρειάζεται αυτό
  το socket· ένδειξη κάτω ορίου για το παράθυρό σας όταν η καταγραφή το χάσει.
- **Bufferbloat** — υπερμεγέθεις ουρές σε routers γεμίζουν υπό φόρτο, φουσκώνοντας το RTT
  (άρα και το BDP) αντί να απορρίπτουν πακέτα νωρίς.
  *Παράδειγμα: ping 15 ms σε ηρεμία που σκαρφαλώνει στα 200 ms κατά τη λήψη είναι bufferbloat.*
- **CUBIC / BBR** — αλγόριθμοι ελέγχου συμφόρησης στον *αποστολέα*. Ο CUBIC (η προεπιλογή)
  μειώνει στο μισό τον ρυθμό του σε απώλεια και ανακάμπτει αργά σε μακρινές διαδρομές· ο BBR
  μοντελοποιεί το εύρος ζώνης και το RTT της διαδρομής, και ανέχεται ελαφρές απώλειες πολύ
  καλύτερα.
